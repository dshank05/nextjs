...
async function handleGet(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const validSortFields = ['id', 'return_date', 'total_amount', 'status', 'fy', 'customer_name']
    ...
    if (needsPostSorting) {
      ...
      const result = await Promise.all([
        ...
        prisma.salex_returns.count({ where })
      ])
      returns = result[0]
      total = result[1]
    } else {
    ...
    const invoiceIds = Array.from(new Set(returns.map(r => r.invoicex_id).filter(Boolean)))
    const returnIds = returns.map(r => r.id)

    // Get invoice and customer details
    const [invoiceData, itemCounts] = await Promise.all([
      // Get invoice details
      invoiceIds.length > 0 ? prisma.invoicex.findMany({
      ...
      returnIds.length > 0 ? prisma.salex_return_items.groupBy({
        by: ['salex_return_id'],
        where: { salex_return_id: { in: returnIds } },
        _count: { id: true }
      }) : Promise.resolve([])
    ])
    ...
    const itemCountMap = new Map(itemCounts.map(ic => [ic.salex_return_id, ic._count.id]))

    // Enhanced returns with customer info
    let enhancedReturns = returns.map((returnRecord) => {
      const invoice = invoiceMap.get(returnRecord.invoicex_id)
      ...
      return {
        id: returnRecord.id,
        invoicex_id: returnRecord.invoicex_id, // Add this for filtering
        return_no: `SX-${String(returnRecord.id).padStart(3, '0')}`, // SX for salex returns
    ...
    if (customer && customer !== '') {
      ...
      if (!isNaN(customerNum)) {
        ...
        enhancedReturns = enhancedReturns.filter(ret => {
          const invoice = invoiceData.find(inv => inv.id === ret.invoicex_id);
    ...
    console.error('Salex returns fetch error:', error)
    res.status(500).json({
      message: 'Failed to fetch salex returns data',
...
async function handlePost(req: NextApiRequest, res: NextApiResponse) {
  try {
    const {
      ...
      items // Array of { invoice_itemx_id, return_qty, return_reason_id, unit_price, notes? }
    } = req.body

    // Validation
    if (!invoicex_id || !items || items.length === 0) {
      return res.status(400).json({
        message: 'Invoicex ID and items are required'
      })
    }
    ...
    // Get invoicex details to validate
    ...
    if (!invoicex) {
      return res.status(400).json({
        message: 'Invalid invoicex selected - invoice does not exist'
      })
    }

    // Calculate totals (no tax for salex)
    let totalAmount = 0
    ...
    // Calculate refund amount (same as total for salex - no tax)
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const invoiceItemIds = items.map((item: any) => parseInt(item.invoice_itemx_id))
      ...
      const returnRecord = await tx.salex_returns.create({
        data: {
      ...
      for (const item of processedItems) {
        ...
        const invoiceItem = invoiceItems.find(ii => ii.id === item.invoice_itemx_id)
      ...
      // Update return_status for the invoicex
    ...
    console.error('Salex return processing error:', error)
    res.status(500).json({
