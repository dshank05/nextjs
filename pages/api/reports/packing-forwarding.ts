  ...
  try {
    ...
    if (transactionType === 'all' || transactionType === 'sale') {
      ...
      const sales = await prisma.invoice.findMany({
        ...
        select: {
          ...
          packing_forwarding_qty: true,
          packing_forwarding_rate: true,
      ...
      sales.forEach(sale => {
        results.push({
          ...
          qty: Number(sale.packing_forwarding_qty || 0),
          rate: Number(sale.packing_forwarding_rate || 0),
          pf_amount: Number(sale.packing_forwarding_total || 0),
    ...
    if (transactionType === 'all' || transactionType === 'purchase') {
      ...
      purchases.forEach(purchase => {
        results.push({
          ...
          qty: Number(purchase.packing_forwarding_qty || 0),
          rate: Number(purchase.packing_forwarding_rate || 0),
          pf_amount: Number(purchase.packing_forwarding_total || 0),
    ...
    results.sort((a, b) => {
      if (sortBy === 'date') {
        ...
        return sortOrder === 'desc' ? b.pf_amount - a.pf_amount : a.pf_amount - b.pf_amount;
      }
      return 0;
    });
    ...
    // Calculate summary
    const summary = {
      total_pf: results.reduce((sum, r) => sum + r.pf_amount, 0),
      total_transactions: total,
      avg_pf: total > 0 ? results.reduce((sum, r) => sum + r.pf_amount, 0) / total : 0
    };
    ...
    return res.status(500).json({ error: 'Failed to fetch packing/forwarding report' });
  }
}
