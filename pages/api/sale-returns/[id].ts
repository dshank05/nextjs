...
async function handleGet(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // Get the return record with invoice and customer
    const returnRecord = await prisma.sale_returns.findUnique({
      where: { id: returnId },
      select: {
        id: true,
        invoice_id: true,
    ...
    // Get invoice details
    const invoice = returnRecord.invoice_id ? await prisma.invoice.findUnique({
    ...
    // Get ALL invoice items from the original invoice (not just returned ones)
    const allInvoiceItems = await prisma.invoiceitems.findMany({
      where: {
    ...
    const bills = [{
      ...
      bill_reference: invoice.invoice_no?.toString() || 'Sale Return',
      invoice_date: invoice.invoice_date ? new Date(invoice.invoice_date * 1000).toISOString().split('T')[0] : '',
    ...
    const response = {
      return: {
        id: returnRecord.id,
        return_no: `SR-${String(returnRecord.id).padStart(3, '0')}`,
...
async function handlePut(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const existingReturn = await prisma.sale_returns.findUnique({
      where: { id: returnId },
      select: {
        payment_status: true,
        invoice_id: true,
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const currentReturnItems = await tx.sale_return_items.findMany({
        where: { sale_return_id: returnId },
        select: {
          invoice_item_id: true,
          return_qty: true
      ...
      // Get current return
      const currentReturn = await tx.sale_returns.findUnique({
        where: { id: returnId },
      ...
      const processedItems = items.map((item: any) => {
        ...
        const BUSINESS_STATE_CODE = 9 // Uttar Pradesh
        let cgst = 0, sgst = 0, igst = 0
        if (item.customer_state_code === BUSINESS_STATE_CODE) {
          cgst = taxAmount / 2
          sgst = taxAmount / 2
        } else {
          igst = taxAmount
...
async function handleDelete(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.sale_return_items.deleteMany({
        where: { sale_return_id: returnId }
      })

      await tx.sale_returns.delete({
        where: { id: returnId }
      })
    })
