...
export default function SaleReturnCreatePage() {
  ...
  // Ref to track if we're initializing customer (to prevent multiple API calls)
  const isInitializingCustomer = useRef(false);
  ...
  const [allLoadedBills, setAllLoadedBills] = useState<SaleBill[]>([]);
  ...
  useEffect(() => {
    ...
    // Skip if we're initializing customer (prevents duplicate calls)
    if (isInitializingCustomer.current) return;

    const timer = setTimeout(() => {
      loadCustomerBills(customer.id, 1, billSearchTerm, dateFrom, dateTo);
    }, 300);

    return () => clearTimeout(timer);
  }, [billSearchTerm, dateFrom, dateTo, customer?.id, isLoadingEditData, isEditMode, bills.length]);
  ...
  const loadCustomerBills = async (customerId: string, page = 1, search = '', fromDate = '', toDate = '', isLoadMore = false) => {
    ...
    try {
      ...
      if (billsResponse.ok) {
        ...
        throw new Error('Failed to load customer bills');
      }
    } catch (error) {
      console.error('Error loading customer data:', error);
      showSnackbar('error', 'Failed to load customer data');
      if (!isLoadMore) {
        setBills([]);
        setAllLoadedBills([]);
      }
  ...
  const handleCustomerSelect = async (customerId: string | null) => {
    ...
    if (selectedCustomer) {
      ...
      isInitializingCustomer.current = true;
  ...
  const loadInvoiceForReturn = async (invoiceId: string) => {
    ...
    try {
      ...
      // Wait for customers to load if not already loaded
      let attempts = 0;
      while (customers.length === 0 && attempts < 10) {
      ...
      showSnackbar('error', error instanceof Error ? error.message : 'Failed to load invoice');
      setLoading(false);
    }
  };
            ...
            {customer && (
              <div className="mt-4 flex items-center justify-between bg-slate-800/50 rounded-lg p-4">
                <div className="flex items-center gap-4">
                  ...
                  <button
                    onClick={async () => {
                      if (!customer?.id || !loadedDateRange.from) return;
              ...
              <div className="border-t border-slate-600 pt-6 mb-6">
                ...
                {filteredBills.length === 0 && (
                  <div className="text-center py-8 text-slate-400">
                    No bills found for the selected customer and filters
                  </div>
                )}
                ...
                {pagination.totalPages > 1 && (
                  <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-600">
                    ...
                    <div className="flex items-center space-x-2">
                      <button
                        onClick={() => loadCustomerBills(customer!.id, pagination.page - 1, appliedFilters.search, appliedFilters.from_date, appliedFilters.to_date)}
