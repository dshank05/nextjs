...
export default function CustomerBalanceLogsPage() {
  ...
  }, [selectedCustomer, selectedColumn]);
  ...
  const fetchCurrentBalance = async () => {
    ...
    try {
      const response = await fetch(`/api/reports/customer-balance-logs?customer_id=${selectedCustomer}&get_balance=true`);
      if (response.ok) {
        const data = await response.json();
        if (data.balance) {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/reports/customer-balance-logs?${params}`);
      if (response.ok) {
        const data = await response.json();
        setLogs(data.data);
      }
      ...
      <div className="card">
          ...
          <div className="overflow-x-auto">
            ...
            {logs.length === 0 && !loading && (
              <div className="text-center py-8 text-slate-400">
                No balance changes found for this customer.
              </div>
            )}
          </div>
        )}
        ...
        {!selectedCustomer && (
          <div className="text-center py-12 text-slate-400">
            ...
            <p className="text-lg">Please select a customer to view balance audit log</p>
          </div>
        )}
      </div>
    </div>
