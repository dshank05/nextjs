...
interface MechanicSaleRecord {
  type: string;
  reference_no: string;
  date: number;
  customer_name: string;
  mechanic_name: string;
  mechanic_city: string;
  total_amount: number;
  commission: number;
  formattedDate: string;
}

export default function MechanicSalesReport() {
  const [data, setData] = useState<MechanicSaleRecord[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/reports/mechanic-sales?${params}`);
      if (response.ok) {
    ...
    <div className="space-y-6">
      <div className="card">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-white">Mechanic Sales Report</h2>
          <ExportMenu
            data={data}
            columns={[
              ...
              { key: 'mechanic_name', label: 'Mechanic', enabled: true },
              { key: 'mechanic_city', label: 'City', enabled: true },
              { key: 'total_amount', label: 'Total Amount', enabled: true },
              { key: 'commission', label: 'Commission', enabled: true }
            ]}
            config={{
              title: 'Mechanic Sales Report',
              fileName: `Mechanic_Sales_Report_${getLocalDateString()}`
            }}
          />
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-4">
          ...
          <div>
            ...
            <SearchableSelect
              options={mechanics}
              selectedValue={filters.mechanicId}
              onSelectionChange={(value) => setFilters(prev => ({ ...prev, mechanicId: value || '' }))}
        ...
        <div className="overflow-x-auto relative">
                ...
                <tr key={idx}>
                  ...
                  <td>
                    <span className={`px-2 py-1 rounded-full text-xs ${
                      record.type === 'Sale' ? 'bg-green-600' : 'bg-blue-600'
                    } text-white`}>
                      {record.type}
                    </span>
                  </td>
                  ...
                  <td className="text-slate-300">{record.mechanic_name}</td>
                  <td className="text-slate-300">{record.mechanic_city}</td>
          ...
          {data.length === 0 && !loading && (
            <div className="text-center py-8 text-slate-400">
              No mechanic sales found with the current filters.
            </div>
          )}
        </div>
