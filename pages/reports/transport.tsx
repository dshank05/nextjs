...
export default function TransportCostReport() {
  const [data, setData] = useState<TransportCostRecord[]>([]);
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <ExportMenu
            ...
            columns={[
              ...
              { key: 'transport_company', label: 'Transport Company', enabled: true },
              { key: 'freight_amount', label: 'Freight Amount', enabled: true },
              { key: 'total_amount', label: 'Total Amount', enabled: true }
            ]}
            config={{
              title: 'Transport Cost Report',
              fileName: `Transport_Cost_Report_${getLocalDateString()}`
            }}
          />
        </div>
        ...
        {summary && (
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-slate-800 p-4 rounded-lg">
              <div className="text-sm text-slate-400 mb-1">Total Freight Charges</div>
              <div className="text-2xl font-bold text-white">₹{summary.total_freight.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</div>
            </div>
            <div className="bg-slate-800 p-4 rounded-lg">
            ...
            <div className="bg-slate-800 p-4 rounded-lg">
              <div className="text-sm text-slate-400 mb-1">Average Freight</div>
              <div className="text-2xl font-bold text-white">₹{summary.avg_freight.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</div>
            </div>
          </div>
        )}
        ...
        <div className="overflow-x-auto relative">
          ...
          <table className="table">
            <thead>
              <tr>
                ...
                <th className="text-right">Freight Amount</th>
            ...
            <tbody>
              {data.map((record, idx) => (
                <tr key={idx}>
                  ...
                  <td className="text-slate-300">{record.transport_company || '-'}</td>
                  <td className="text-right font-semibold text-white">₹{record.freight_amount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
          ...
          {data.length === 0 && !loading && (
            <div className="text-center py-8 text-slate-400">
              No transport costs found with the current filters.
            </div>
          )}
        </div>
