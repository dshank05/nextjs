...
export default function CustomerLedgerPage() {
  ...
  const [selectedCustomer, setSelectedCustomer] = useSessionStorage<string>('customer-ledger-customer', '');
  const [dateFrom, setDateFrom] = useSessionStorage<string>('customer-ledger-dateFrom', '');
  const [dateTo, setDateTo] = useSessionStorage<string>('customer-ledger-dateTo', '');
  ...
  useEffect(() => {
    ...
    const storedDateFrom = sessionStorage.getItem('customer-ledger-dateFrom');
    const storedDateTo = sessionStorage.getItem('customer-ledger-dateTo');
  ...
  }, [selectedCustomer, dateFrom, dateTo, pagination.page]);
  
  // ✅ Cleanup sessionStorage on component unmount
  useEffect(() => {
    return () => {
      // Clear all customer-ledger sessionStorage keys when leaving page
      sessionStorage.removeItem('customer-ledger-customer');
      sessionStorage.removeItem('customer-ledger-dateFrom');
      sessionStorage.removeItem('customer-ledger-dateTo');
    };
  }, []);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/reports/customer-ledger-accounting?${params}`);
  ...
  const selectedCustomerName = customers.find(c => c.id.toString() === selectedCustomer)?.billing_name || '';
  ...
  return (
    <div className="space-y-6">
      <div className="card">
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="text-sm text-slate-400">
              {selectedCustomerName && (
                <span className="font-medium text-white">Ledger for: {selectedCustomerName}</span>
              )}
            </div>
            <div className="flex items-center gap-3">
              ...
              config={{
                title: `Customer Ledger - ${selectedCustomerName}`,
                fileName: `Customer_Ledger_${selectedCustomerName}_${getLocalDateString()}`
              }}
              />
            </div>
          </div>
        ...
        <div className="overflow-x-auto relative">
          ...
          {!selectedCustomer ? (
            <div className="text-center py-12 text-slate-400">
              ...
              <p className="text-lg">Please select a customer to view ledger</p>
            </div>
          ) : (
          ...
          {selectedCustomer && accountingEntries.length === 0 && !loading && (
        ...
        {/* Pagination */}
        {pagination.totalPages > 1 && selectedCustomer && (
        ...
        {selectedCustomer && accountingEntries.length > 0 && (
